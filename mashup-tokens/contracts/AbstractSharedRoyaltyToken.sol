...
contract AbstractSharedRoyaltyToken is ISharedRoyaltyToken, ERC721 {
     ...
     */
    function franchisorBalanceOf(address franchisor)
        public
        view
        notZeroAddr(franchisor)
        returns (uint256)
    {
    ...
    function withdrawPayment(
        address payable franchisor,
        uint256 count,
        uint256 tokenId
    ) public notZeroAddr(franchisor) {
        Token storage token = _tokens[tokenId];

        uint256 withdrawPaymentsLeft = franchisorWithdrawPaymentsLeft(
            franchisor,
            tokenId
        );
        uint256 lastWithdrawnIndex = token
            .franchisorNextWithdrawIndex[franchisor];
        uint256 withdrawCount = lastWithdrawnIndex.add(count) >
        ...
        token.franchisorNextWithdrawIndex[franchisor] = lastWithdrawnIndex.add(
            withdrawCount
        );
     ...
     * @param franchisor address to withdraw payment for
     */
    function withdrawPayment(address payable franchisor, uint256 tokenId)
        public
        notZeroAddr(franchisor)
    {
        withdrawPayment(
        ...
        returns (uint256)
    {
        ...
        require(
            _isFranchisor(franchisor, tokenId),
            "Payments can only be withdrawn by franchisors"
        );

        return
            token.payments.length.sub(
                token.franchisorNextWithdrawIndex[franchisor]
            );
    }
    ...
    function _addFranchisor(
        ...
        _franchisorTokensCount[franchisor] = _franchisorTokensCount[franchisor]
            .add(1);
     ...
     */
    function _isFranchisor(address franchisor, uint256 tokenId)
        internal
        view
        returns (bool)
    {
